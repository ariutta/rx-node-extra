...
var sologger = require('../test/sologger.js');
...
function ask(source, getPrompts, createIterable) {
  ...
  return itemSource
    .concatMap(function(item) {
      ...
      var i = 0;
      sologger.bind('item').log(item);
      return Rx.Observable.while(
          function() {
            sologger.bind('while_b4 -- i').log(i);
            sologger.bind('finalPromptIndex').log(finalPromptIndex);
            return i++ <= finalPromptIndex;
          },
          inquirerSource.take(1)
        )
        .reduce(function(accumulator, response) {
          sologger.bind('accumulatorb4').log(JSON.stringify(accumulator));
          sologger.bind('response').log(JSON.stringify(response));
          ...
          var nextIndex = i;
          sologger.bind('doOnNext -- nextIndex').log(nextIndex);
          sologger.bind('answer').log(answer);

          // TODO does this handle the last askOption?
          var nextPrompt = prompts[nextIndex];
          if (nextPrompt) {
            var nextWhen = nextPrompt.when;
            if (nextWhen) {
              sologger.bind('nextWhen').log(nextWhen.toString());
              sologger.bind('answers').log(answers);
              sologger.bind('nextWhen(answers)').log(nextWhen(answers));
            }
            if (nextWhen && !nextWhen(answers)) {
              finalPromptIndex -= 1;
              sologger.bind('decremented finalPromptIndex').log(finalPromptIndex);
            }
          }

          accumulator.answers = answers;
          sologger.bind('accumulator_after').log(JSON.stringify(accumulator));
          return accumulator;
        }, starter);
    });
}
